...
- `wm-config-registry` — FastAPI backend контейнер для registry write/read API
- `wm-config-registry-outbox-worker` — отдельный worker-контейнер, который
...
- пересобирает локальные image `wm-config-registry`, `grafana`, `kafka-connect`
...
`tools/wm_clickhouse_migrations/migrations` и применяются forward-only CLI:
...
`wm-config-registry-outbox-worker` запускается отдельным контейнером и является
...
```bash
wm-config-registry publish-config-outbox-worker
```
