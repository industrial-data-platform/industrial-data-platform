...
| Блок | Назначение |
| --- | --- |
| `Service inventory` | tenants, assets, agents, sources, observed points; configured points after current inventory read model |
...
First increment должен явно различать метрики:

- `observed_points` можно считать из `telemetry_latest_v1`;
- `configured_points` нельзя считать production-ready метрикой из
  `points_json` на каждый refresh; она появляется только после
  `current_point_inventory` или эквивалентного service read model.
