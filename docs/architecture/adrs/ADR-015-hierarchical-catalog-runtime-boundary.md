# ADR-015: Hierarchical Catalog and Asset Graph boundary comparison
...
самостоятельной моделью. Первый источник изменений — ручной internal
`/backoffice` workflow.
...
Второй use case — target `Asset Graph Registry`:
...
должен читать и доставлять данные. Target `Asset Graph Registry` отвечает на
вопрос: что существует в реальном мире, как это связано и какие логические
атрибуты можно наблюдать или вычислять.
...
Референсы показывают, что промышленные asset graph/catalog решения почти всегда
...
| --- | --- |
| Azure ADT object graph / DTDL models | Model instance и relationships образуют graph. Relationships имеют семантику вроде containment или functional links. Azure также явно отделяет model graph от device state; telemetry в DTDL не является основным способом хранения incoming device data. |
...
| [Eclipse BaSyx architecture](https://eclipse.dev/basyx/architecture/) / IDTA AAS metamodel | AAS дает digital identity asset-а и submodels для static/dynamic data. Dynamic submodels дают unified access to live asset data; RelationshipElement связывает referable/external references. |
| [Eclipse Ditto basic concepts](https://eclipse.dev/ditto/basic-overview.html) | Ditto хорошо иллюстрирует current-state object document: attributes для metadata, features/properties для state. Это ближе к latest-state API, чем к богатой industrial asset graph ontology. |
...
target asset graph layer требует отдельного словаря понятий:
`asset_graph_node`, `attribute`, `relationship`, `telemetry binding`, `unit`,
`quality/status`, `computed attribute` и application views.
...
дерево навигации и authoring projection, а не полноценный asset graph layer.

### Use case B: Asset Graph Registry

Target `Asset Graph Registry` — это отдельная capability поверх data platform.

Она должна рассматривать как first-class concepts:

- `asset_graph_node_code` и `object_type`;
...
простой `Catalog V1`; это первый slice `Asset Graph Registry`, даже если UI
начинает с дерева.
...
- название `Catalog` может незаметно расширить Config Registry до asset graph
  layer;
- Web Monitoring может получить premature dependency на Config Registry;
- будущий вынос в Asset Graph Registry service потребует миграции schema/API/use
  cases.

### Вариант B: отдельный Asset Graph Registry service/package в monorepo

Asset Graph Registry получает собственный runtime boundary, API, package и
ownership, но остается частью `Industrial Data Platform` monorepo.
...
- естественнее для future `Asset Graph Registry`;
...
- нужен межмодульный contract между Config Registry и Asset Graph Registry;
...
Он плохо подходит как основной редактор дерева или target asset graph:
...
Если выбирается вариант B, нужен отдельный internal Asset Graph Registry API и
отдельный internal UI, специально сделанный для редактирования дерева/графа.
Config Registry SQLAdmin может оставаться только adjacent admin surface.
...
3. Future enrichment связывает technical series с
   `asset_graph_node.attribute`.
4. Web Monitoring и Alarm Management смогут читать latest/history не только по
   raw point, но и по semantic path: `asset_graph_node_code`/`attribute_key`,
   `object_type`, unit, location/path и graph relations.
...
- первый slice — navigation tree или asset graph?
...
embedded или separate slice, но явно без target asset graph promises.

Если задача — создать основу для object attributes, telemetry bindings,
semantic monitoring и alarm rules, нужно обсуждать `Asset Graph Registry` как
отдельную domain capability. В этом случае отдельный service/package должен
быть полноценным first option, а не future optimization.
...
- LikeC4 показывает отдельный `Asset Graph Registry` container.
