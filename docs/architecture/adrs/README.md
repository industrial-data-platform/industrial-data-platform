...
| `ADR-003-buffering-and-delivery.md` | partially superseded | Исторически фиксирует SQLite outbox/state. Детали delivery/MQTT и storage contracts уточнены в `ADR-005` и `docs/contracts/edge-telemetry-agent/`. |
...
| `ADR-005-mqtt-event-transport.md` | accepted | Фиксирует MQTT 5 как edge transport, telemetry/status/config topics, QoS/retain policy и topic-derived routing. Детальные schemas в `docs/contracts/edge-telemetry-agent/`. |
...
| Вопрос | Читать |
| --- | --- |
| Как агент получает конфигурацию? | `ADR-008`, затем `docs/contracts/edge-telemetry-agent/` |
| Как устроена идентичность данных? | `ADR-004` |
| Как публикуется telemetry в MQTT? | `ADR-005`, затем `docs/contracts/edge-telemetry-agent/mqtt-topic-tree.v1.md` |
| Как работает KNX MVP? | `ADR-001`, `ADR-002` |
| Зачем SQLite на edge? | `ADR-003`, затем `docs/contracts/edge-telemetry-agent/sqlite-storage.v1.md` |
