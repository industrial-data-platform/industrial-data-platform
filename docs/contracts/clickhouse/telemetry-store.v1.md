...
| `service_latest_agent_status_v1` | Service read model: latest agent status по `tenant_id + asset_id + agent_id` | `service_latest_agent_status_mv_v1` |
| `service_latest_source_connection_v1` | Service read model: latest source connection state по `tenant_id + asset_id + agent_id + source_id` | `service_latest_source_connection_mv_v1` |
| `service_point_inventory_v1` | Service read model: current observed point inventory по raw point identity | `service_point_inventory_mv_v1` |
| `service_telemetry_activity_1m_v1` | Service read model: deduplicated telemetry activity rollup for dashboard counts and top-N | `service_telemetry_activity_1m_mv_v1` |
...
| `agent_status_events_v1` | `service_latest_agent_status_mv_v1` | `service_latest_agent_status_v1` |
| `source_connection_events_v1` | `service_latest_source_connection_mv_v1` | `service_latest_source_connection_v1` |
| `telemetry_events_v1` | `service_point_inventory_mv_v1` | `service_point_inventory_v1` |
| `telemetry_events_v1` | `service_telemetry_activity_1m_mv_v1` | `service_telemetry_activity_1m_v1` |
...
Migration `0002_service_dashboard_read_models.sql` добавляет additive service
read models для internal Grafana dashboard refresh paths. Эти модели не
заменяют correctness-first views как общий API, но снимают production-heavy
inventory/status/activity/top-N panels с регулярного чтения raw/history views.

Dashboard query patterns before `ORDER BY` choice:

- global service overview counts over current observed point inventory;
- latest agent/source health grouped by status/state with stale freshness
  threshold;
- telemetry activity trend and top-N points over dashboard time range;
- selected point event-rate drilldown over service activity rollups;
- drilldown variable discovery narrowed by
  `tenant_id -> asset_id -> source_type -> agent_id -> source_id -> point_key`;
- selected point history still reads bounded correctness-first views where
  value-level history is required.

`service_latest_agent_status_v1`:

- grain: `tenant_id + asset_id + agent_id`
- shape: `AggregatingMergeTree` target populated by incremental MV from
  `agent_status_events_v1`
- state columns:
  `status_state`, `last_status_ts_state`, `last_ingested_at_state`
- read convention: group by grain and read with
  `argMaxMerge(status_state)` / `maxMerge(last_status_ts_state)`
- order key: `(tenant_id, asset_id, agent_id)`

`service_latest_source_connection_v1`:

- grain: `tenant_id + asset_id + agent_id + source_id`
- shape: `AggregatingMergeTree` target populated by incremental MV from
  `source_connection_events_v1`
- state columns:
  `state_state`, `reason_state`, `last_state_ts_state`, `last_ingested_at_state`
- read convention: group by grain and read with
  `argMaxMerge(state_state)`, `argMaxMerge(reason_state)` and
  `maxMerge(last_state_ts_state)`
- order key: `(tenant_id, asset_id, agent_id, source_id)`
...
- grain:
  `tenant_id + asset_id + source_type + agent_id + source_id + point_key + bucket_start`
- shape: `AggregatingMergeTree` target populated by incremental MV from
  `telemetry_events_v1`
- state columns:
  `event_count_state`, `good_count_state`, `uncertain_count_state`,
  `bad_count_state`, `first_ts_state`, `last_ts_state`
- dedup convention: event counts use `uniq*State(idempotency_key)` so Kafka
  replay duplicates do not inflate dashboard activity counts.
- read convention: use `uniqMerge(event_count_state)` and
  `uniqIfMerge(*_count_state)` grouped by dashboard dimensions/time range.
- selected point drilldown convention: filter by
  `tenant_id + asset_id + source_type + source_id + point_key`, include
  `agent_id` when the chained variable is selected, and bound by dashboard time
  range on `bucket_start`.
- partition: `toYYYYMM(bucket_start)`
- order key:
  `(tenant_id, asset_id, source_type, source_id, point_key, bucket_start, agent_id)`
  to keep drilldown identity filters aligned with the telemetry sort-key prefix
  while retaining monthly time partition pruning for overview time windows.
